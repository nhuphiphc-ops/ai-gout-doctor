...
# Replace form feed character with backtick
text = text.replace('\x0clex', '\lex')
# Also fix the missing template literal parts because the regex ate them
# I'll just rewrite the whole bottom nav correctly.
...
text = re.sub(r'<div className="fixed bottom-0 left-0 right-0.*</div>\s*</div>', replacement + '\n    </div>', text, flags=re.DOTALL)
